...
// Load GLTF/GLB only in the browser
let GLTFLoader: any = null;

type ChatTurn = { who: "AI" | "You"; text: string };

// Browser speech types (avoid TS conflicts server-side)
...
export default function AIDirectorWidget() {
  ...
  // Lazy-import GLTFLoader only on client
  useEffect(() => {
    (async () => {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        // Wait until GLTFLoader is present
        let tries = 0;
      ...
      // Procedural jaw (so we can demo talking NOW even if the GLB has no morphs)
  ...
  function ensureAudioGraphFor(el: HTMLAudioElement) {
    ...
    // Recreate graph each time to keep it simple
